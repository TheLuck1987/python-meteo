...
field_names = [
    "temperature_2m", "precipitation", "precipitation_probability", "cape", "wind_speed_10m",
    "wind_gusts_10m", "surface_pressure", "cloud_cover", "relative_humidity_2m", "dew_point_2m",
    "apparent_temperature", "rain", "showers", "snowfall"
]
serie_names_it = [
    "Temperatura", "Precipitazioni", "Prob. Prec.", "CAPE", "Vento", "Raffiche", "Pressione",
    "Copertura Nuvolosa", "Umidità Relativa", "Punto di Rugiada", "Temp. Percepita", "Pioggia",
    "Rovesci", "Neve"
]
DAY_NAMES_IT = {
...
def get_data(latitude, longitude, local_path):
    if os.path.exists(local_path) and (datetime.now() - datetime.fromtimestamp(os.path.getmtime(local_path))).seconds < 3600:
...
def robust_mean(arr):
    ...
    iqr = q3 - q1
    filt = vals[(vals >= q1 - 1.5*iqr) & (vals <= q3 + 1.5*iqr)]
    return np.mean(filt) if len(filt) > 0 else np.median(vals)
...
def create_mid_graph(df, combined_data):
    ...
    for idx, col in enumerate(combined_data.columns[1:]):
        fig.add_trace(go.Scatter(
            x=df["time"], y=combined_data[col], mode="lines",
...
def get_series(df, serie_index, combined_means, pdf_indexed=None):
    ...
    if serie_index == 0 and pdf_indexed is not None:
        # Vectorizzato: prendo date_key e calcolo media storica per tutte le date contemporaneamente
    ...
    if past_values is not None:
        fig.add_trace(go.Scatter(
            x=df["time"], y=past_values, mode="lines",
            name=PAST_MEAN_LABEL, line=dict(width=1.5, dash="longdash", color="red")
        ))
    for f in valid_fields:
        fig.add_trace(go.Scatter(
            x=df["time"], y=df[f], mode="lines", name=f.replace(field_names[serie_index], ""), line=dict(width=1.5)
        ))
    if valid_fields:
        fig.add_trace(go.Scatter(
            x=df["time"], y=df["mean"], mode="lines", name=serie_names_it[serie_index], line=dict(width=3, color="blue")
        ))
    return fig
...
# --- GENERAZIONE PAGINE ---
def generate_forecast_pages(folder_name, latitude, longitude, pdf_indexed):
...
if __name__ == "__main__":
    ...
    pdf_indexed = pdf.pivot(index="month_day_hour", columns="year", values="temperature_2m")

    for folder, cfg in LOCATIONS.items():
        generate_forecast_pages(folder, cfg["lat"], cfg["lon"], pdf_indexed)
